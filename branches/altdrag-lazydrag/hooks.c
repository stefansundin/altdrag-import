...
int alt=0;
int malt=0; //Keeps track of whether the middle mouse button caused the alt press
...
_declspec(dllexport) LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
		if (wParam == WM_LBUTTONDOWN && alt && !move) {
			...
			if (!sharedsettings.LazyDrag && !(GetAsyncKeyState(VK_LMENU)&0x8000) || sharedsettings.LazyDrag && malt && !(GetAsyncKeyState(VK_MBUTTON)&0x8000) && !(GetAsyncKeyState(VK_RBUTTON)&0x8000)) {
				alt=0;
				malt=0;
				UnhookMouse();
				return CallNextHookEx(NULL, nCode, wParam, lParam);
		...
		else if ((wParam == WM_MBUTTONDOWN || wParam == WM_RBUTTONDOWN) && sharedsettings.LazyDrag && !alt) {
			alt=1;
			malt=1;
		}
		...
		else if ((wParam == WM_MBUTTONUP || wParam == WM_RBUTTONUP) && sharedsettings.LazyDrag && alt && malt) {
			alt=0;
			malt=0;
			return 1;
		}
...
int UnhookMouse() {
	if (!mousehook || sharedsettings.LazyDrag) {
		//Mouse not hooked or LazyDrag enabled
		return 1;
	}
...
BOOL APIENTRY DllMain(HINSTANCE hInstance, DWORD reason, LPVOID reserved) {
	if (reason == DLL_PROCESS_ATTACH) {
		...
		if (!sharedsettings_loaded) {
			...
			//Cursor
			GetPrivateProfileString(APP_NAME,L"Cursor",L"1",txt,sizeof(txt)/sizeof(wchar_t),inipath);
			...
			//LazyDrag
			GetPrivateProfileString(APP_NAME,L"LazyDrag",L"0",txt,sizeof(txt)/sizeof(wchar_t),inipath);
			swscanf(txt,L"%d",&sharedsettings.LazyDrag);
			if (sharedsettings.LazyDrag) {
				HookMouse();
			}
